...
function obtenerFiltroNsfw() {
    return '&exclude_tags=nsfw,nudity,eroge,hentai,adult,sexual-content,porn,pornography,sex,18';
}
...
async function cargarDatosRawg() {
    try {
        ...
        const datosEstrenos = await respEstrenos.json();
        
        if (carruselInner && carruselIndicators) {
            const limpiosCarrusel = limpiarJuegos(datosCarrusel.results);
            const carruselMezclado = limpiosCarrusel.sort(() => 0.5 - Math.random()).slice(0, 6);
            renderizarCarrusel(carruselMezclado);
        }
        
        const contenedorEstrenosInner = document.getElementById('contenedor-estrenos-inner');
        if (contenedorEstrenosInner && datosEstrenos.results) {
            const limpiosEstrenos = limpiarJuegos(datosEstrenos.results);
            const estrenosMezclados = limpiosEstrenos.sort(() => 0.5 - Math.random()).slice(0, 15);
            renderizarCarruselEstrenos(estrenosMezclados);
        }

        if (contenedorGrilla) {
            const limpiosGrilla = limpiarJuegos(datosGrilla.results);
            const grillaMezclada = limpiosGrilla.sort(() => 0.5 - Math.random()).slice(0, 10);
            renderizarGrilla(grillaMezclada, 'contenedor-juegos-grilla');
        }
...
function renderizarCarruselEstrenos(juegos) {
    const contenedor = document.getElementById('contenedor-estrenos-inner');
    if (!contenedor) return;
    
    contenedor.innerHTML = '';
    
    // Agrupar juegos en bloques de 5
                ...
                <div class="card card-game h-100" style="cursor: pointer;" onclick="window.location.href=window.location.pathname.includes('/HTML/') ? 'Detalles.html?id=${juego.id}' : 'HTML/Detalles.html?id=${juego.id}'">
                    <div class="position-relative">
                        ...
                        <button class="btn btn-outline-danger btn-sm position-absolute top-0 start-0 m-2 rounded-circle btn-deseo d-flex align-items-center justify-content-center p-0" 
                                ...
                                data-id="${juego.id}"
                                onclick="event.stopPropagation(); if(window.toggleDeseo) window.toggleDeseo('${juego.id}', '${juego.name.replace(/'/g, "\\'")}', '${precioAleatorio}', '${juego.background_image}', this)"><span class="material-symbols-outlined fs-5">favorite_border</span></button>
                    </div>
                    <div class="card-body d-flex flex-column text-start">
                        ...
                        <div class="mt-auto d-flex flex-column gap-2">
                            ...
                            <button class="btn btn-principal btn-sm w-100 d-flex justify-content-center align-items-center gap-1" onclick="event.stopPropagation(); if(window.agregarAlCarrito) window.agregarAlCarrito('${juego.id}', '${juego.name.replace(/'/g, "\\'")}', '${precioAleatorio}', '${juego.background_image}')"><span class="material-symbols-outlined" style="font-size: 18px;">shopping_cart</span> Añadir al Carrito</button>
                        </div>
                    </div>
                </div>
            </div>`;
    ...
    // Sincronizar corazones al terminar
    if(window.actualizarCorazonesUI) window.actualizarCorazonesUI();
}

function renderizarGrilla(juegos, contenedorId = 'contenedor-juegos-grilla') {
    const contenedorSeleccionado = document.getElementById(contenedorId);
    ...
    juegos.forEach(juego => {
        ...
        `;
        contenedorSeleccionado.innerHTML += tarjetaHtml;
    });
